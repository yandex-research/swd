...
class FlowMatchingSolver:
    ...
    def __init__(
        ...
        """
        noise_scheduler.set_timesteps(num_timesteps)
        ...
        if num_boundaries == 0:
            ...
            if boundaries is None:
                self.boundary_idx = torch.linspace(
                    0, len(noise_scheduler.timesteps), num_boundaries + 1, dtype=int
                )
            else:
    ...
    def sample_end_boundary_idx(self, batch_of_start_idx):
        ...
        idx = torch.argmax(
            mask[
                [self.num_boundaries - 1] + list(range(0, self.num_boundaries - 1)), :
            ],
            dim=0,
        )
    ...
    def _get_previous_scale(self, batch_of_scales):
        scales_idx = torch.argmax(
            (batch_of_scales[:, None] == self.scales).int(), dim=1
        )
    ...
    def downscale_to_previous_and_upscale(self, sample, scales):
        ...
        if scales[0].item() == self.min_scale:
            sample = torch.nn.functional.interpolate(
                sample, size=int(scales[0].item()), mode="area"
            )
            return sample

        previous_scales = self._get_previous_scale(scales)
        sample = torch.nn.functional.interpolate(
            sample, size=int(previous_scales[0].item()), mode="area"
        )
        sample = torch.nn.functional.interpolate(
            sample, size=int(scales[0].item()), mode="bicubic"
        )
        return sample
    ...
    def sampling(
        ...
        if do_scales:
            latent = torch.randn(
                (len(prompt_embeds), 16, int(self.min_scale), int(self.min_scale)),
                generator=generator,
        ...
        while True:
            ...
            latent = latent - noise_pred * sigma[:, None, None, None]
            ...
            if do_scales:
                if int(self.scales[k - 1]) != int(next_scale):
                    latent = torch.nn.functional.interpolate(
                        latent, size=int(next_scale), mode="bicubic"
                    )

                noise = torch.randn(
                    *latent.shape, generator=generator, device=latent.device
                )
                latent = (
                    sigma_next[:, None, None, None] * noise
                    + (1.0 - sigma_next[:, None, None, None]) * latent
                )
                k += 1
                try:
